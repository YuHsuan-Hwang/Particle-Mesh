...
const double dt = 0.005;    // time interval
    ...
    for (int k = 0; k < gridNk ; k++) {
        ...
        for (int j = 0; j < gridN; j++) {
            ...
            for (int i = 0; i < (gridN/2+1) ; i++) {
                ...
                //rho_k[k + gridN * gridN * j + gridN * i][0] *= fac;
                //rho_k[k + gridN * gridN * j + gridN * i][1] *= fac;
                /*
                if (fabs(fac) < 1e-14)
                {
...
void compute_accelerations(double* acc_x, double* acc_y, double* acc_z, double* phi) {
    ...
    double scaling_factor = -2 * dx * gridN * gridN * gridNk  ;
    #pragma omp parallel
    {
    ...
    for (int p = 0; p < N; p++) {
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp sections
            {
                ...
                {
                    x[p] = x[p] + vx[p] * 0.5 * dt;
                    vx[p] = vx[p] + a_x[k * gridN * gridN + j * gridN + i] * dt;
                    x[p] = x[p] + vx[p] * 0.5 * dt;
                }
                #pragma omp section
                {
                    y[p] = y[p] + vy[p] * 0.5 * dt;
                    vy[p] = vy[p] + a_y[k * gridN * gridN + j * gridN + i] * dt;
                    y[p] = y[p] + vy[p] * 0.5 * dt;
                }
                #pragma omp section
                {
                    z[p] = z[p] + vz[p] * 0.5 * dt;
                    vz[p] = vz[p] + a_z[k * gridN * gridN + j * gridN + i] * dt;
                    z[p] = z[p] + vz[p] * 0.5 * dt;
                }
            }
        }
